...
# Play OpenArm Lift Cube Task (local)
# Usage: ./scripts/play_lift.sh [--checkpoint /path/to/model.pt] [--num_envs 16] [additional args...]
...
if [[ ! "$*" =~ "--checkpoint" ]]; then
    LATEST_CHECKPOINT=$(find "${REPO_ROOT}/logs/rsl_rl" -name 'model_*.pt' -path '*lift*' 2>/dev/null | sort -V | tail -1)
    if [ -n "${LATEST_CHECKPOINT}" ]; then
        echo "Using latest checkpoint: ${LATEST_CHECKPOINT}"
        EXTRA_ARGS="${EXTRA_ARGS} --checkpoint ${LATEST_CHECKPOINT}"
    else
        echo "Warning: No checkpoint found. Train a model first with ./scripts/train_lift.sh"
        exit 1
    fi
fi
