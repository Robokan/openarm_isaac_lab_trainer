...
# Play OpenArm Bimanual Reach Task for Teleoperation (expanded workspace)
# Usage: ./scripts/play_bimanual_reach_for_teleop.sh [--checkpoint /path/to/model.pt] [--num_envs 1] [additional args...]
...
if [[ ! "$*" =~ "--checkpoint" ]]; then
    LATEST_CHECKPOINT=$(find "${REPO_ROOT}/logs/rsl_rl/openarm_bi_reach_teleop" -name 'model_*.pt' 2>/dev/null | sort -V | tail -1)
    if [ -z "${LATEST_CHECKPOINT}" ]; then
        LATEST_CHECKPOINT=$(find "${REPO_ROOT}/logs/rsl_rl" -name 'model_*.pt' -path '*bi_reach*' 2>/dev/null | sort -V | tail -1)
    fi
    if [ -n "${LATEST_CHECKPOINT}" ]; then
        echo "Using latest checkpoint: ${LATEST_CHECKPOINT}"
        EXTRA_ARGS="${EXTRA_ARGS} --checkpoint ${LATEST_CHECKPOINT}"
    else
        echo "Warning: No checkpoint found. Train a model first with ./scripts/train_bimanual_reach_for_teleop.sh"
        exit 1
    fi
fi
