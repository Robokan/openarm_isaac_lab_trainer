...
# Play OpenArm Open Drawer Task (local)
# Usage: ./scripts/play_drawer.sh [--checkpoint /path/to/model.pt] [--num_envs 16] [additional args...]
...
if [[ ! "$*" =~ "--checkpoint" ]]; then
    LATEST_CHECKPOINT=$(find "${REPO_ROOT}/logs/rsl_rl" -name 'model_*.pt' -path '*drawer*' -o -name 'model_*.pt' -path '*cabinet*' 2>/dev/null | sort -V | tail -1)
    if [ -n "${LATEST_CHECKPOINT}" ]; then
        echo "Using latest checkpoint: ${LATEST_CHECKPOINT}"
        EXTRA_ARGS="${EXTRA_ARGS} --checkpoint ${LATEST_CHECKPOINT}"
    else
        echo "Warning: No checkpoint found. Train a model first with ./scripts/train_drawer.sh"
        exit 1
    fi
fi
